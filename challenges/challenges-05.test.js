...
const listFoods = (recipe) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        let arr2 = arr[i].slice(first + 1, arr[i].length);
        let second = arr2.indexOf(" ");
        result.push(arr2.slice(second + 1, arr2.length));
    }
    return result;
};
...
const splitFoods = (recipe) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        result.push(arr2.slice(2, arr2.length).join(" "));
    }
    return result;
};
...
const stepActions = (recipe) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        let arr2 = arr[i].split(" ");
        result.push(arr2.slice(0, 1).join(" "))
    }
    return result;
};
...
//         x = str;
//     } else {
//         x = str.slice(0, str.length - numberOfCharacters)
//     }
//     return x;
// };
...
const extractVowels = (str) => {
    ...
    let strArr = str.split('')
    for (let i = 0; i < strArr.length; i++) {
        while (arr.includes(strArr[i])) {
            vowels = vowels + (strArr[i]);
            strArr.splice(i, 1);
        }
    }
    let final = strArr.join("");
    result.push(final);
    result.push(vowels.split('').sort().join(''));
    return result;
